...
'''
1. 关闭了所有的print结果，要看具体信息直接上output.txt和图表查看
2. 增加return项，返回mAP
3. 关闭了tqdm，避免和自己的重合（如果自己没写tqdm可以打开，直接搜tqdm定位即可）
4. 修改了输出output文件夹的位置，定位为os.path.join(root_dir, 'output')
5. 关闭mAP显示，但是保存所有图(line925 to_show = False)
6. 修复检测没有detections导致的plot的bug（前期不存），用try-except处理
'''
